...
export default function MechanicsScenarioPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (!s) { setError(t("mechanics.notFound")); return; }
      setScenario(s);
      setAllIds(all.map(x => x.id));
    }).catch(() => setError(t("mechanics.loadFailed")))
  ...
  if (error || !scenario) {
    return (
      <div style={{ padding: 24, textAlign: "center", paddingTop: 80 }}>
        ...
        <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 20 }}>{error ?? t("mechanics.notFound")}</div>
    ...
    <div style={{ paddingBottom: 32 }}>
      <div style={{ background: T.navy, padding: "52px 20px 20px", color: T.white }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 20, fontWeight: 700 }}>{t("mechanics.title")}</span>
        </div>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.6)" }}>
          {t("mechanics.crewSize", { count: Number(scenario.crewSize) })}
        </div>
      </div>
